...
/**
 * Constructs a GA4 BetaAnalyticsDataClient object. 
 */
export default async function getBetaAnalyticsDataClient(): Promise<BetaAnalyticsDataClient> {
    // We read the authentication key JSON from the environment variable, then write it to a temporary 
    // serverside file which is automatically read by GoogleAuth based on the filepath at the 
    // GOOGLE_APPLICAITON_CREDENTIALS environment variable, which is written to by the function
    // after placing the file in the temporary directory.

    // Fetch and validate the Google Key
    const keyString = process.env.GOOGLE_CREDENTIALS
    if (!keyString) {throw new Error("GOOGLE_CREDENTIALS environment variable is not defined.")}
