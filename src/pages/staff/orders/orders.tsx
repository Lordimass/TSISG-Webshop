...
export function OrderManager() {
    // TODO: Don't fetch orders until user is authenticated, this is a security risk
                ...
                <p>
                    ...
                    {order.fulfilled ? <b> FULFILLED</b> :
                        timeDifference > overdue_threshold ?
                            <b>OVERDUE BY {timeDifference - overdue_threshold} DAYS</b> :
...
function OrderDropdown() {
    async function toggleFulfilment() {
        ...
        try { // Toggle order fulfilment
            ...
            notify(`Something went wrong toggling order fulfilment! ${JSON.stringify(e)}`)
        }
    ...
    async function setDeliveryCost(e: FormEvent<HTMLFormElement>) {
        e.preventDefault()
        if (!order) return
        ...
        if (+val <= 0) {
            notify("Cost must be a valid number greater than 0!");
            return
        }
        try {
            await callRPC("update_delivery_cost", supabase, {order_id: order.id, new_cost: val}, notify)
        } catch (e) {
            notify(`Something went wrong setting delivery cost! ${JSON.stringify(e)}`)
        }
    ...
    return (<>
        ...
        <div className="order-products">
            {order.products ? order.products.map((prod) => {
                ...
                You don't have permission to see the products attached to this order! This is likely a mistake,
